...
$(document).ready(function(){       
	...
	var options = $(this).attr('title');
	    ...
	    if ($("input[type=checkbox][name^='"+id+"']").prop('checked')) {	    	
                    $('#codfor'+options).prop('disabled', '');
                    $('#numtit'+options).prop('disabled', '');
                    $('#dtvenc'+options).prop('disabled', '');
                    $('#vlrcor'+options).prop('disabled', '');                                        	    	
                    $('#emp'+options).prop('disabled', '');
                    $('#codnap'+options).prop('disabled', '');
                    $('#fil'+options).prop('disabled', '');
		} else {
                    $('#codfor'+options).prop('disabled', 'disabled');
                    $('#numtit'+options).prop('disabled', 'disabled');
                    $('#dtvenc'+options).prop('disabled', 'disabled');
                    $('#vlrcor'+options).prop('disabled', 'disabled');                                        			
        ...
        if (check.length == 0) {
            ...
            bootbox.dialog({
                ...
                message: '<div class="row">  ' +
                        ...
                        'Quantidade de Titulos: <strong>' + check.length + '</strong>' +
                        '<h3></div>' +                        
                        '</div> </div>',
                buttons: {
                    cancel: {
                    ...
                    success: {
                        ...
                        'callback': function () {
                            dataString = $("#form_apr_par").serialize();
                            $.blockUI({
                            ...
                            $.ajax({
                                type: "POST",
                                url: "<?php echo base_url(); ?>index.php/parceria/aprovar",
                                data: dataString,
                                success: function (data) {
                                    ...
                                    $('#mRetorno').on('hidden.bs.modal', function () {
                                        window.location = "<?php echo base_url(); ?>index.php/parceria";
                                    });
                                }
                            });
                        }
...
<div class="panel panel-custom">
	 ...
	 <?php 								
		$attribute = array('id' => 'form_apr_par', 'name' => 'form_apr_par'); 
