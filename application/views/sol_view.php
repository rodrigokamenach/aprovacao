...
$(function(){
	$("#busadt").click(function(){
    	...
    	//$('#regiao').html('');
      	...
      	$.ajax({
        	type: "POST",
        	url: "<?php echo base_url();?>index.php/sol/carreg_sc",
        	data: dataString,        	
        	datatype:'json',
...
// 	}
	...
	function jVePendente(emp, numapr, rotnap, numsol, codfil, seqsol){    		
		//antes de abrir a janela, preciso carregar os dados do cliente e preencher os campos dentro do modal			
		carregaDadosPendentes(emp, numapr, rotnap, numsol, codfil, seqsol);    		
		$('#mVePendente').modal('show');
	}

	function carregaDadosPendentes(emp, numapr, rotnap, numsol, codfil, seqsol){
		$.ajax({  
            type: "POST",  
            url : '<?php echo base_url(); ?>index.php/sol/busca_pendente/'+emp+'/'+numapr+'/'+rotnap+'/'+numsol+'/'+codfil+'/'+seqsol,              
            success: function(data){
                //alert(data);  
            	$('#tbpen').html(data); 
            }  
	...
	function jAprova() {
		...
		if (check.length == 0) {
			bootbox.alert({
				  size: 'small',
			      "message": '<h5 class="alert alert-danger"><i class="fa fa-exclamation-triangle"></i> Selecione ao menos uma solicitação!</h5>',
...
//     						'Valor Total: <strong>R$ ' +soma + '</strong></h3>' + 
			    ...
			    buttons:{
				    ...
				    success: {
			        		...
			        		'callback'  : function(){			        		
			        		      dataString = $("#form_apr_sol").serialize();
			        		      $.blockUI({ 
			        		      ...
			        		      $.ajax({
			        		        type: "POST",
			        		        url: "<?php echo base_url();?>index.php/sol/aprovar",
			        		        data: dataString,
			        		        success: function(data){			        		        	
			        		        	$.unblockUI(); 					        		        	        
			        	            	$('#retorno').html(data);			        	            				        	            		        	            	      			        		            			        		     
			        		            ...
			        		            $('#mRetorno').on('hidden.bs.modal', function() {
			        		            	window.location="<?php echo base_url();?>index.php/sol"; 
			        		            });          
			        		        }
			        		      }); 
			        		}
		...
		<div class="col-md-12">
			<div class="panel panel-custom">
				<div class="panel-heading">
			    	<h5><i class="fa fa-search"></i> Aprovação de Solicitações</h5>
				</div>
